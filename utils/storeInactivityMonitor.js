// Caminho: utils/storeInactivityMonitor.js
        ...
        try {
            ...
            for (const settings of guildsWithMonitor.rows) {
                ...
                const inactiveCarts = await dbClient.query(
                    `SELECT * FROM store_carts WHERE guild_id = $1 AND (status = 'open' OR status = 'payment') AND last_activity_at < NOW() - INTERVAL '1 hour' * $2`,
...
module.exports = { checkInactiveCarts, updateCartActivity: (channelId) => db.query('UPDATE store_carts SET last_activity_at = NOW() WHERE channel_id = $1', [channelId]) };