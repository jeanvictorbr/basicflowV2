...
const { formatDuration } = require('../../utils/pontoUtils.js'); // Ajuste o path se necessário

module.exports = {
    customId: 'ponto_end_service',

    async execute(interaction) {
        // 1. BLINDAGEM: Evita duplo clique ou erro de "Unknown Interaction"
        ...
        try {
            // 2. BUSCA SESSÃO ABERTA
            const sessionRes = await db.query(`
                SELECT id, start_time, total_pause_ms, last_pause_start, status 
                FROM ponto_sessions 
            ...
            // ==================================================================================
            // 3. CÁLCULO PRECISO DO TEMPO (Com desconto de pausas)
            // ==================================================================================
            ...
            await db.query(`
                ...
                SET end_time = NOW(), 
                    ...
                    total_pause_ms = $1
                WHERE id = $2
            `, [currentTotalPause, session.id]);

            // 5. ATUALIZAR O RANKING (O que estava faltando!)
            // Usa UPSERT (Insert ou Update se já existir)
            await db.query(`
            ...
            // 7. ATUALIZA O PAINEL (Para voltar a aparecer "Iniciar")
            await updatePontoPainel(interaction, userId);

            // 8. RESPOSTA FINAL (Usando seu utils para formatar se disponível, ou fallback manual)
            let timeString;
            try {
                timeString = formatDuration(finalTotalMs); // Tenta usar seu utils
            } catch (e) {
                // Fallback manual caso o utils falhe ou não tenha a função
            ...
            if (!interaction.replied) {
                await interaction.editReply({ content: "❌ Ocorreu um erro interno ao salvar seus dados. Contate o suporte." });
            }
        }
    }
};